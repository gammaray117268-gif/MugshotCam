(function() {
  ...
  const App = {
    state: {
      ...
      detainee: null,
      session: {
        photos: { frontHalf: null, leftSide: null, rightSide: null, fullBody: null },
        originalPhotos: { frontHalf: null, leftSide: null, rightSide: null, fullBody: null },
        timestamp: null
      },
      currentAngleIndex: 0,
      stream: null,
    ...
    angles: [
      { key: 'frontHalf', label: 'Front View (Half-Body)', guide: 'Face the camera directly. Keep head and shoulders within the guide box.', cssClass: '' },
      { key: 'leftSide', label: 'Left Side View (Half-Body)', guide: 'Turn 90 degrees to your left. Show left profile within the guide box.', cssClass: '' },
      { key: 'rightSide', label: 'Right Side View (Half-Body)', guide: 'Turn 90 degrees to your right. Show right profile within the guide box.', cssClass: '' },
      { key: 'fullBody', label: 'Front View (Full-Body)', guide: 'Stand straight facing the camera. Ensure full body fits within the guide box.', cssClass: 'full-body' }
    ],
    ...
    async checkSession() {
      try {
        ...
        if (data.success && data.officer) {
          this.state.officer = data.officer;
    ...
    async handleRegister() {
      ...
      try {
        ...
        this.showMessage('loginMessage', 'Registration failed. Please try again.', 'error');
      }
    },
    ...
    handleStartCapture: function() {
      ...
      this.state.detainee = { fullName, offense, dateOfArrest: arrestDate, bookingId };
      this.state.session = {
        photos: { frontHalf: null, leftSide: null, rightSide: null, fullBody: null },
        originalPhotos: { frontHalf: null, leftSide: null, rightSide: null, fullBody: null },
        timestamp: null
    ...
    startCamera: async function() {
      ...
      const enableCamera = () => {
        ...
        if (statusEl) { statusEl.textContent = 'Camera ready'; statusEl.style.color = ''; }
      ...
      try {
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Camera request timed out')), 10000));
        this.state.stream = await Promise.race([
          navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 1280 }, height: { ideal: 720 }, facingMode: this.state.facingMode }, audio: false }),
          timeoutPromise
        ]);
        ...
        const waitForVideo = () => new Promise((resolve) => {
          ...
          const check = () => {
            if (this.state.cameraCallId !== callId) { resolve(false); return; }
            const ready = video.readyState >= video.HAVE_ENOUGH_DATA && video.videoWidth > 0 && video.videoHeight > 0;
            if (ready) { setStatus(`Video ready: ${video.videoWidth}x${video.videoHeight}`); resolve(true); }
            else if (Date.now() - start > maxWait) { setStatus('Video timeout. Enabling capture anyway.'); resolve(false); }
        ...
        if (err.name === 'NotAllowedError') msg = 'Camera permission denied. Please allow camera access and refresh.';
        else if (err.name === 'NotFoundError') msg = 'No camera found on this device.';
        else if (err.name === 'NotReadableError') msg = 'Camera is in use by another app. Close other apps and retry.';
        else if (err.name === 'OverconstrainedError') msg = 'Camera does not meet requirements. Try switching camera mode.';
        else if (err.message === 'Camera request timed out') msg = 'Camera request timed out. Please check permissions and retry.';
    ...
    switchCameraMode: function() {
      ...
      const video = document.getElementById('webcamVideo');
      if (label) label.textContent = this.state.facingMode === 'user' ? 'Front' : 'Back';
      if (video) {
        if (this.state.facingMode === 'environment') video.classList.add('back-camera');
        else video.classList.remove('back-camera');
      }
      this.stopCamera();
      this.startCamera();
    },
    ...
    updateCaptureUI: function() {
      ...
      document.getElementById('captureStepLabel').textContent = `Step ${this.state.currentAngleIndex + 1} of 4: ${angle.label}`;
      ...
      if (angle.cssClass === 'full-body') cropBox.classList.add('full-body');
      else cropBox.classList.remove('full-body');
    ...
    capturePhoto: function() {
      ...
      try {
        ...
        if (!vw || !vh) {
          if (statusEl) { statusEl.textContent = 'Video not ready yet. Please wait a moment and try again.'; statusEl.style.color = 'var(--color-danger)'; }
          return;
        }
        ...
        if (statusEl) { statusEl.textContent = 'Capture failed: ' + err.message; statusEl.style.color = 'var(--color-danger)'; }
      }
    },
    ...
    populateReview: function() {
      ...
      const setReviewPhoto = (key, imgId) => {
        ...
        if (img && this.state.session.photos[key]) img.src = this.state.session.photos[key];
      };
    ...
    async saveSession() {
      ...
      const photosPayload = [];
      const types = ['frontHalf', 'leftSide', 'rightSide', 'fullBody'];
      for (const type of types) {
        if (this.state.session.photos[type]) {
          photosPayload.push({ type, base64: this.state.session.photos[type], isOriginal: 0 });
        }
        if (this.state.session.originalPhotos[type]) {
          photosPayload.push({ type, base64: this.state.session.originalPhotos[type], isOriginal: 1 });
        }
      }
      ...
      try {
        const res = await fetch(API_BASE + '/records.php', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            bookingId: this.state.detainee.bookingId,
            detaineeName: this.state.detainee.fullName,
            offense: this.state.detainee.offense,
            dateOfArrest: this.state.detainee.dateOfArrest,
        ...
        const data = await res.json();
        if (data.success) {
          this.showMessage('exportMessage', 'Record saved successfully.', 'success');
        } else {
          this.showMessage('exportMessage', 'Failed to save record: ' + data.message, 'error');
        }
      } catch (e) {
        console.error('Save session failed:', e);
        this.showMessage('exportMessage', 'Failed to save record. Please check your connection.', 'error');
      }
    },
